...
pub fn sc_reason(
    req: web::Json<model::Request>,
    db: web::Data<db::Pool>,
    //db_sfa: web::Data<db::SFA>,
    db_sfa: web::Data<AppState>
) -> impl Future<Item = HttpResponse, Error = AppError> {
    web::block(move || {
        let mut conn = db.get().unwrap();
        let mut conn_sfa = db_sfa.sfa.get().unwrap();
        println!("sfa {:?}", conn_sfa);
        let data = req.data.get();
        ...
        match db::TrxLogs(&mut conn, &req.into_inner()) {
            Ok(_) => {
                match db::TrxReason(&mut conn, &result) {
                    Ok(_) => {
                        match ScClient::Calculate(&mut ScClient::default(), &mut conn_sfa, &mut conn, &result.sc_id, &result.id) {
                            Ok(ok) => Ok(ok),
                            Err(e) => Err(e),
                        }
                    },
